 ...
 *
 * $Id: flowid.cc 43 2010-11-17 21:52:00Z yww4 $
 *
 */
...
FlowId::FlowId() {
    ...
    id_num = 0;
    ip_v = 4;
	if(packet_debug) cout << "Created null flowid\n";
}
...
FlowId::FlowId ( uint32_t ip_src, uint32_t ip_dst, uint16_t port_src,
    ...
    id_num = id;
    ip_v = 4;
	if(packet_debug) cout << "Created v4 flowid with values (1) id = " << id << "\n";
...
FlowId::FlowId ( uint8_t ip_src[16], uint8_t ip_dst[16], uint16_t port_src,
    ...
    id_num = id;
    ip_v = 6;
	if(packet_debug) cout << "Created v6 flowid with values (2)\n";
}
